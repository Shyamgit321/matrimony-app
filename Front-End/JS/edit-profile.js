...
// DOB
// ===============================
if (user.dob && user.dob !== "None") {
  const d = new Date(user.dob);

  if (!isNaN(d.getTime())) {
    dobDate.value = String(d.getDate());
    dobMonth.value = String(d.getMonth() + 1);
...
// CLOUDINARY IMAGE
// ===============================
editPhotoPreview.src = data.image;
...
const dob =
`${dobYear.value}-` +
`${dobMonth.value.padStart(2, "0")}-` +
`${dobDate.value.padStart(2, "0")}`;
